...
#SBATCH --exclusive
#SBATCH --output=MPI_weak_THIN_static.out
...
output_dir="/u/dssc/pdarol00/Final-assigments-FHPC-course-2022-2023/exercise1/data/weak_MPI/static"
...
do  
    ...
    for j in {1..5..1}
    do
        mpirun -n $mpi_task --map-by $MAPBY --bind-to $BINDTO ./main.x -r -k $ksize -e $e -n $nsteps -s $s  > output_THIN_static_MPI_weak.txt
        time_value=$(grep -o 'Time: [0-9.]*' output_THIN_static_MPI_weak.txt | cut -d' ' -f2)
        thread_value=$(grep -o 'Threads: [0-9.]*' output_THIN_static_MPI_weak.txt | cut -d' ' -f2)
        echo "$ksize,$mpi_task,$time_value,$thread_value" >> $filename
    done
done

rm output_THIN_static_MPI_weak.txt # Remove useless temporary file
