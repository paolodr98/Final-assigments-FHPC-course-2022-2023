...
#SBATCH --exclusive
#SBATCH --output=OMP_Init_EPYC_static.out
...
do  
    ...
    do        
        ...
        do
            ...
            for j in {1..5..1}
            do
                mpirun -n $mpi_task --map-by $MAPBY --bind-to $BINDTO ./main.x -r -k $ksize -e $e -n $nsteps -s $s  > output_EPYC_static_openMP.txt
                time_value=$(grep -o 'Time: [0-9.]*' output_EPYC_static_openMP.txt | cut -d' ' -f2)
                thread_value=$(grep -o 'Threads: [0-9.]*' output_EPYC_static_openMP.txt | cut -d' ' -f2)
...
rm output_EPYC_static_openMP.txt # Remove useless temporary file
